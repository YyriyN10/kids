...
function getStates(zipStringList) {
    let stList = [];
    for (var i = 0; i < zipStringList.length; i++) {
        zipString = zipStringList[i];
...
def get_nums(arr, st):
    arr_new = [x[1] for x in arr if x[0] == st]
    if len(arr_new) == 1: 
        arr_new = arr_new[0]
    else: 
        return None
    return [int(y) for y in arr_new]
...
if __name__ == '__main__':
    ...
    info = {}
    states = []
    states = get_states()([x for x in data])

    for index, zipcode in enumerate(data):
        nums = get_nums(data[zipcode], states[index])
        if (nums):
            info[zipcode] = nums
            print(zipcode, info[zipcode])

    write_json(info, 'test_out.json')